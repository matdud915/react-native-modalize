...
handlePosition = 'outside', disableScrollIfPossible = true, avoidKeyboardLikeIOS = react_native_1.Platform.select({
    ...
    android: false,
    default: true,
}), keyboardAvoidingBehavior = 'padding', keyboardAvoidingOffset, panGestureEnabled = true, panGestureComponentEnabled = false, tapGestureEnabled = true, closeOnOverlayTap = true, closeSnapPointStraightEnabled = true, direction = 'top', 
// Animations
openAnimationConfig = {
...
onOpen, onOpened, onClose, onClosed, onBackButtonPress, onPositionChange, onOverlayPress, onLayout, }, ref) => {
    ...
    const computedHeight = fullHeight - (slideFromTop ? 0 : handleHeight) - (devices_1.isIphoneX ? 34 : 0);
    ...
    const translateY = React.useRef(new react_native_1.Animated.Value(slideFromTop ? -screenHeight : screenHeight))
        .current;
    ...
    const handleAnimateClose = (dest = 'default', callback) => {
        ...
        let toValue = toInitialAlwaysOpen && alwaysOpen ? (modalHeightValue || 0) - alwaysOpen : screenHeight;
        if (slideFromTop) {
            const max = modalHeight && modalHeight > 0
                ? modalHeight
                : Array.from(layouts).reduce((acc, cur) => acc + (cur === null || cur === void 0 ? void 0 : cur[1]), 0);
            const flat = react_native_1.StyleSheet.flatten(modalStyle);
    ...
    const handleChildren = ({ nativeEvent }, type) => {
        ...
        const thresholdProps = translationY * translationMultiplier > threshold && beginScrollYValue === 0;
         ...
         */
        if (nativeEvent.oldState === react_native_gesture_handler_1.State.BEGAN && !slideFromTop) {
    ...
    const handleGestureEvent = react_native_1.Animated.event([{ nativeEvent: { translationY: dragY } }], {
        ...
        listener: ({ nativeEvent: { translationY } }) => {
            var _a;
            if (panGestureAnimatedValue) {
                const offset = ((_a = alwaysOpen !== null && alwaysOpen !== void 0 ? alwaysOpen : snapPoint) !== null && _a !== void 0 ? _a : slideFromTop) ? 0 : -endHeight;
    ...
    const renderHandle = () => {
        ...
        const handleStyles = [styles_1.default.handle, handlePosition];
    ...
    const renderChildren = () => {
        ...
        const minDist = libraries_1.isRNGH2() ? undefined : ACTIVATED * activationMultiplier;
        return (React.createElement(react_native_gesture_handler_1.PanGestureHandler, { ref: panGestureChildrenRef, enabled: panGestureEnabled, simultaneousHandlers: [nativeViewChildrenRef, tapGestureModalizeRef], shouldCancelWhenOutside: false, onGestureEvent: handleGestureEvent, minDist: minDist, activeOffsetY: ACTIVATED * activationMultiplier, activeOffsetX: ACTIVATED, onHandlerStateChange: handleChildren },
